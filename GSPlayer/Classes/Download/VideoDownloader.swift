...
extension VideoDownloader: VideoDownloaderHandlerDelegate {
    func handler(_: VideoDownloaderHandler, didReceive response: URLResponse) {
    ...
    func handler(_: VideoDownloaderHandler, didReceive data: Data, isLocal _: Bool) {
        delegate?.downloader(self, didReceive: data)
    }

    func handler(_: VideoDownloaderHandler, didFinish error: Error?) {
